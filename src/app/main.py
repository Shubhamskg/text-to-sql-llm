...
def sql_copilot(question,language_model=llm, max_attempts=3):
    result='FAIL'
    output='Output: \n'
        ...
        while attempt <= max_attempts:
            ...
            if query_result[:5] == 'Error': #if the result returns an error message
                if attempt >= max_attempts:
                    ...
                    result = 'FAIL'
                    output = output+f"Unable to execute the SQL query after {max_attempts} attempts."
                    break
...
st.set_page_config(page_title="Text to SQL Demo")
st.header("What would you like to know from your data?: ")

input=st.text_input("Input: ",key="input")
response=sql_copilot(input)
